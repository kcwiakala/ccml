...
{ 
  Network net;
  neuron_layer_ptr_t l1 = std::make_shared<FullyConnectedLayer>(2, 7, Activation::sigmoid());
  neuron_layer_ptr_t l2 = std::make_shared<FullyConnectedLayer>(7, 1, Activation::sigmoid());
  neuron_layer_ptr_t l3 = std::make_shared<FullyConnectedLayer>(64, 1, Activation::sigmoid());
  net.push(l1);
  net.push(l2);
  // net.push(l3);

  ccml::Initializer initializer = Initializer::normal(0, 0.2);
